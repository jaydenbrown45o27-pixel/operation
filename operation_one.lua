...
local ESP = loadstring(game:HttpGet("https://raw.githubusercontent.com/0x3u/Scripts/refs/heads/main/Operation%20One/Esp"))()

local SupportSA = pcall(function() assert(getactors) end)

shared.lyzn = {
    aimbot = false, aimbot_key = Enum.UserInputType.MouseButton2, aimbot_fov = 150,
    aimbot_smoothing = 0.15, show_fov = true, headlock = true, wall_check = true,
    chams = false, chams_color = Color3.fromRGB(0, 255, 255), chams_material = "ForceField",
    no_recoil = false, recoil_amt = 0,
    gadget_esp = false,
    headlock_aim = false, headlock_fov = 120, headlock_sens = 0.35,
}
...
shared._lyzn_cleanup = function()
    ...
    if shared._lyzn_fov_circle then pcall(function() shared._lyzn_fov_circle:Remove() end) end
    for _, draw in pairs(shared._lyzn_gadget_drawings or {}) do pcall(function() draw:Remove() end) end
    shared._lyzn_gadget_drawings = nil
...
local function isAlive(char) if not char then return false end local h = char:FindFirstChildWhichIsA("Humanoid") return h and h.Health > 0 end
local function isEnemy(plr) if plr == LocalPlayer then return false end local m, t = LocalPlayer.Team, plr.Team if m and t and m == t then return false end return true end

local function isVisible(fromPos, targetPos, ignorePlr)
    local dir = (targetPos - fromPos)
    local params = RaycastParams.new()
    params.FilterType = Enum.RaycastFilterType.Exclude
    local ignoreList = {Camera}
    if LocalPlayer.Character then table.insert(ignoreList, LocalPlayer.Character) end
    if ignorePlr and ignorePlr.Character then table.insert(ignoreList, ignorePlr.Character) end
    ...
    if ignorePlr then
        ...
        if model then
            ...
            if p == ignorePlr then return true end
        end
    end
    return false
end

local function LoadNoRecoil()
    if not SupportSA then return end
    pcall(function()
        run_on_actor(getactors()[1], [[
            local Players = game:GetService("Players")
            local LocalPlayer = Players.LocalPlayer
            local GunModule = require(game:GetService("ReplicatedStorage").Modules.Items.Item.Gun)
            local recoilFunction = GunModule.recoil_function
            if _G.recoilHooked then return end
            _G.recoilHooked = true
            local constants = getconstants(recoilFunction)
            local recoilIndex = nil
            for i = 1, #constants do
                if type(constants[i]) == "string" and constants[i] == "pc" then
                    setconstant(recoilFunction, i, "made by lyzn")
                    recoilIndex = i + 1
                    break
                end
            end
            while task.wait() do
                if recoilIndex then
                    local enabled = LocalPlayer:GetAttribute("Mods_NoRecoilEnabled") or false
                    local amt = LocalPlayer:GetAttribute("Mods_RecoilAmt") or 0
                    setconstant(recoilFunction, recoilIndex, enabled and (amt / 100) or 1)
                end
            end
        ]])
    end)
...
shared._lyzn_gadget_drawings = shared._lyzn_gadget_drawings or {}
local GadgetDrawings = shared._lyzn_gadget_drawings

local function IsOwnGadget(gadget)
    local owner = gadget:GetAttribute("Owner") or gadget:FindFirstChild("Owner")
    if owner then
        local ownerName = tostring(owner)
        if ownerName == LocalPlayer.Name or ownerName == LocalPlayer.DisplayName then return true end
        local ownerPlr = Players:FindFirstChild(ownerName)
        if ownerPlr and ownerPlr.Team == LocalPlayer.Team then return true end
    end
    return false
end

local function AddGadget(gadget)
    if not gadget then return end
    local name = gadget.Name:lower()
    if name:find("claymore") or name:find("landmine") or name:find("mine") then
        if not GadgetDrawings[gadget] then
            local sq = Drawing.new("Square")
            sq.Thickness = 2.5; sq.Filled = false; sq.Visible = false
            GadgetDrawings[gadget] = sq
        end
    end
end

for _, obj in ipairs(workspace:GetDescendants()) do AddGadget(obj) end
table.insert(connections, workspace.DescendantAdded:Connect(function(child)
    task.wait(0.5); AddGadget(child)
end))

local SavedProperties = {}

local function ApplyChams(part)
    if not part:IsA("BasePart") or SavedProperties[part] then return end
    SavedProperties[part] = {Color = part.Color, Material = part.Material, Transparency = part.Transparency}
    part.Color = shared.lyzn.chams_color
    part.Material = Enum.Material[shared.lyzn.chams_material] or Enum.Material.ForceField
    part.Transparency = 0.5
end

local function ToggleChams(state)
    if not state then
        for p, prop in pairs(SavedProperties) do
            if p and p.Parent then
                p.Color = prop.Color; p.Material = prop.Material; p.Transparency = prop.Transparency
            end
        end
        SavedProperties = {}
        return
    ...
    if vmFolder then
        table.insert(connections, vmFolder.ChildAdded:Connect(function(child)
            if child.Name == "LocalViewmodel" then
                task.wait(0.2)
                for _, v in ipairs(child:GetDescendants()) do ApplyChams(v) end
            end
        end))
        local current = vmFolder:FindFirstChild("LocalViewmodel")
        if current then
            for _, v in ipairs(current:GetDescendants()) do ApplyChams(v) end
        end
    end
end

shared._lyzn_fov_circle = Drawing.new("Circle")
shared._lyzn_fov_circle.Color = Color3.fromRGB(255,255,255); shared._lyzn_fov_circle.Thickness = 1
shared._lyzn_fov_circle.Filled = false; shared._lyzn_fov_circle.Transparency = 0.6
shared._lyzn_fov_circle.NumSides = 64; shared._lyzn_fov_circle.Radius = shared.lyzn.aimbot_fov
shared._lyzn_fov_circle.Visible = false

local aiming, aimTarget = false, nil
local mobileAiming = false

local function getViewmodelForPlayer(plr)
...
local function getAimTarget()
    local closest, closestDist = nil, shared.lyzn.aimbot_fov
    local center = Vector2.new(Camera.ViewportSize.X / 2, Camera.ViewportSize.Y / 2)
    for _, plr in ipairs(Players:GetPlayers()) do
        if not isEnemy(plr) then continue end
        local char = plr.Character; if not char or not isAlive(char) then continue end
        local vm = getViewmodelForPlayer(plr)
        local tp
        if shared.lyzn.headlock then
            tp = vm and vm:FindFirstChild("head") or char:FindFirstChild("HumanoidRootPart")
        else
            tp = vm and vm:FindFirstChild("torso") or char:FindFirstChild("HumanoidRootPart")
        end
        if not tp then continue end
        local sp, on = Camera:WorldToViewportPoint(tp.Position)
        if not on then continue end
        local screenPos = Vector2.new(sp.X, sp.Y)
        local d = (screenPos - center).Magnitude; if d >= closestDist then continue end
        if shared.lyzn.wall_check then
            if not isVisible(Camera.CFrame.Position, tp.Position, plr) then continue end
        end
        closestDist = d; closest = {player = plr, part = tp}
    end
    return closest
end
...
table.insert(connections, UserInputService.InputBegan:Connect(function(input, gpe)
    ...
    if shared.lyzn.aimbot and (input.UserInputType == shared.lyzn.aimbot_key or input.KeyCode == Enum.KeyCode.CapsLock) then
        aiming = true
    end
end))
table.insert(connections, UserInputService.InputEnded:Connect(function(input)
    if isMobile then return end
    if input.UserInputType == shared.lyzn.aimbot_key or input.KeyCode == Enum.KeyCode.CapsLock then
...
local function GetClosestHeadInFOV()
    ...
    local maxDistSq = shared.lyzn.headlock_fov ^ 2
    local Viewmodels = workspace:FindFirstChild("Viewmodels")
    if not Viewmodels then return nil end
    for _, model in ipairs(Viewmodels:GetChildren()) do
        if model.Name ~= "LocalViewmodel" and model:FindFirstChild("torso") then
            local head = model:FindFirstChild("head")
            if head and head.Transparency < 1 then
                local pos, onScreen = Camera:WorldToViewportPoint(head.Position)
                if onScreen then
                    local screenPos = Vector2.new(pos.X, pos.Y)
                    local distanceSq = (screenPos - center).Magnitude ^ 2
                    if distanceSq <= maxDistSq and distanceSq < minDist then
                        minDist = distanceSq; closest = head
                    end
                end
            end
        end
...
table.insert(connections, RunService.RenderStepped:Connect(function()
    ...
    shared._lyzn_fov_circle.Visible = shared.lyzn.aimbot and shared.lyzn.show_fov
    shared._lyzn_fov_circle.Position = Vector2.new(Camera.ViewportSize.X / 2, Camera.ViewportSize.Y / 2)
    shared._lyzn_fov_circle.Radius = shared.lyzn.aimbot_fov

    local isAiming = isMobile and mobileAiming or aiming
    if isAiming and shared.lyzn.aimbot then
        aimTarget = getAimTarget()
        if aimTarget and aimTarget.part and aimTarget.part.Parent then
            local tPos = aimTarget.part.Position
            local cPos = Camera.CFrame.Position
            local dir = (tPos - cPos).Unit
            local s = shared.lyzn.aimbot_smoothing
            local cL = Camera.CFrame.LookVector
            local nL = (cL + (dir - cL) * math.clamp(s, 0.01, 1)).Unit
            Camera.CFrame = CFrame.lookAt(cPos, cPos + nL)
        end
    end

    if shared.lyzn.headlock_aim then
        local targetHead = GetClosestHeadInFOV()
        if targetHead then
            local headPos = Camera:WorldToViewportPoint(targetHead.Position)
            local center = Camera.ViewportSize / 2
            local dx = (headPos.X - center.X) * shared.lyzn.headlock_sens
            local dy = (headPos.Y - center.Y) * shared.lyzn.headlock_sens
            pcall(function() mousemoverel(dx, dy) end)
        end
    end

    if shared.lyzn.gadget_esp then
        for gadget, square in pairs(GadgetDrawings) do
            if not gadget or not gadget.Parent then
                if square then square.Visible = false end
                GadgetDrawings[gadget] = nil; continue
            end
            local color = IsOwnGadget(gadget) and Color3.fromRGB(0, 255, 100) or Color3.fromRGB(255, 0, 0)
            local root = gadget.PrimaryPart or gadget:FindFirstChildWhichIsA("BasePart")
            if not root then continue end
            local pos, onScreen = Camera:WorldToViewportPoint(root.Position)
            if onScreen then
                local dist = (Camera.CFrame.Position - root.Position).Magnitude
                if dist < 500 then
                    local size = math.clamp(800 / dist, 10, 50)
                    square.Size = Vector2.new(size, size)
                    square.Position = Vector2.new(pos.X - size/2, pos.Y - size/2)
                    square.Color = color; square.Visible = true
                else square.Visible = false end
            else square.Visible = false end
        end
    else
        for _, square in pairs(GadgetDrawings) do
            if square then square.Visible = false end
        end
    end
end))

local Window = Library:CreateWindow({ Title = "Lyzn Hub", SubTitle = "Operation One" })

local Combat = Window:CreateTab("Aimbot")

local aimSection = Combat:CreateSection("Aimbot", "Left")
aimSection:AddToggle({ Text = "Enable Aimbot", Default = shared.lyzn.aimbot, Callback = function(v) shared.lyzn.aimbot = v end })
aimSection:AddToggle({ Text = "Headlock Target", Default = shared.lyzn.headlock, Callback = function(v) shared.lyzn.headlock = v end })
aimSection:AddToggle({ Text = "Wall Check", Default = shared.lyzn.wall_check, Callback = function(v) shared.lyzn.wall_check = v end })
aimSection:AddToggle({ Text = "Show FOV Circle", Default = shared.lyzn.show_fov, Callback = function(v) shared.lyzn.show_fov = v end })
aimSection:AddSlider({ Text = "FOV Radius", Min = 30, Max = 500, Default = shared.lyzn.aimbot_fov, Decimals = 0, Callback = function(v) shared.lyzn.aimbot_fov = v end })
aimSection:AddSlider({ Text = "Smoothing", Min = 1, Max = 100, Default = math.floor(shared.lyzn.aimbot_smoothing * 100), Decimals = 0, Callback = function(v) shared.lyzn.aimbot_smoothing = v / 100 end })

local headlockSection = Combat:CreateSection("Head Lock", "Right")
headlockSection:AddToggle({ Text = "Enable Head Lock", Default = shared.lyzn.headlock_aim, Callback = function(v) shared.lyzn.headlock_aim = v end })
headlockSection:AddSlider({ Text = "FOV", Min = 30, Max = 400, Default = shared.lyzn.headlock_fov, Decimals = 0, Callback = function(v) shared.lyzn.headlock_fov = v end })
headlockSection:AddSlider({ Text = "Sensitivity", Min = 1, Max = 100, Default = math.floor(shared.lyzn.headlock_sens * 100), Decimals = 0, Callback = function(v) shared.lyzn.headlock_sens = v / 100 end })

local VisualsTab = Window:CreateTab("Visuals")

local skelSection = VisualsTab:CreateSection("Skeleton ESP", "Left")
skelSection:AddToggle({ Text = "ESP Enabled", Default = false, Callback = function(v) ESP.Enabled = v end })
skelSection:AddToggle({ Text = "Skeleton", Default = false, Callback = function(v) ESP.Drawing.Skeleton.Enabled = v end })
skelSection:AddToggle({ Text = "Team Check", Default = false, Callback = function(v) ESP.Drawing.TeamCheck.Enabled = v end })

local boxSection = VisualsTab:CreateSection("Box & Name ESP", "Right")
boxSection:AddToggle({ Text = "Names", Default = false, Callback = function(v) ESP.Drawing.Names.Enabled = v end })
boxSection:AddToggle({ Text = "Distances", Default = false, Callback = function(v) ESP.Drawing.Distances.Enabled = v end })
boxSection:AddToggle({ Text = "Gadget ESP", Default = shared.lyzn.gadget_esp, Callback = function(v) shared.lyzn.gadget_esp = v end })

local WeaponTab = Window:CreateTab("Misc")

local chamsSection = WeaponTab:CreateSection("Weapon Chams", "Left")
chamsSection:AddToggle({ Text = "Enable Chams", Default = false, Callback = function(v) ToggleChams(v) end })
chamsSection:AddDropdown({ Text = "Material", List = {"ForceField", "Neon", "Glass", "Metal"}, Default = shared.lyzn.chams_material, Callback = function(m) shared.lyzn.chams_material = m end })

local modsSection = WeaponTab:CreateSection("Mods", "Left")
modsSection:AddToggle({ Text = "No Recoil", Default = shared.lyzn.no_recoil, Callback = function(v) shared.lyzn.no_recoil = v; LocalPlayer:SetAttribute("Mods_NoRecoilEnabled", v) end })
modsSection:AddSlider({ Text = "Recoil Amount", Min = 0, Max = 100, Default = shared.lyzn.recoil_amt, Decimals = 0, Callback = function(v) shared.lyzn.recoil_amt = v; LocalPlayer:SetAttribute("Mods_RecoilAmt", v) end })